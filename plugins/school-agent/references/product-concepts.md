...
- A draft is a local artifact under its assignment's `Drafts/` directory when assignment context is known. Its provenance records assignment/course identifiers, policy metadata, model IDs, source paths, timestamp, version, and run ID.
- `revise <run-id>` creates another version; it does not edit or replace a pending draft in place.
- `approve <run-id>` promotes the local artifact into that assignment's `Final/` directory. It does not submit to Canvas.
