...
4. Verify the result in the configured vault using the linked public vault-layout guide: each course directory is rooted at stable `course-<CanvasID>` (for example, `course-12345`), not a normalized course-code slug. Synced modules, assignments, announcements, files, and the course manifest are under that root. Use frontmatter to check identity when names or folders are ambiguous.

## Diagnose duplicate weeks or course mix-ups

The v2 vault groups synced course material under `course-<CanvasID>/Week NN - Mon DD/` or `course-<CanvasID>/Milestone NN - Title/` folders. Prep is stored in that period's `Prep/` directory when retrieval selected a matching module; otherwise it may appear in course-level `Other/Prep/`. For duplicate-week reports, compare Canvas-ID root, course identity, period number/date, and frontmatter before treating paths as duplicates. Same week labels in different course roots are expected.
...
2. Check `src/store/paths.ts` for course-path derivation and `src/canvas/sync.ts` / `src/canvas/sync-course.ts` for selection and writes. Current course roots use `course-<CanvasID>`; do not infer a root from the course code or assume older code-slug paths prove current behavior. Verify the actual ID and path before calling two paths a collision.
